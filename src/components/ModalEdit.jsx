...
export default function ModalEdit({ isOpen, closeModal, inputData, judulModal, endpoints }) {
    ...
    function handleUpdate(e) {
        ...
        axios.patch(endpoints['update'], dataDetail, {
            ...
            .then(res => {
                //refresh setelah berhasil di hapus
                                    ...
                                    Object.entries(inputData).map(([index, item]) => (
                                        <div className="mb-6">
                                        {
                                            item.type == "select" ? (
                                                ...
                                                <div>
                                                    <label for={index} class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{index}</label>
                                                    <input type={item.type} name={index} id={index} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500  dark:text-white" value={dataDetail[index]} onChange={handleChange}/>
                                                </div>
                                            )
                                        }
                                        </div>
